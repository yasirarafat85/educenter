...
function render_item_card(array $item, string $type): string
{
    ...
    // ফিচার চেকমার্ক লিস্ট (কোর্স/প্রোডাক্টের নিজস্ব features টেবিল থেকে, সর্বোচ্চ ১০টা)
    $features = [];
    if ($type === 'course') {
        $fs = get_db()->prepare('SELECT feature_text FROM course_features WHERE batch_id = :id ORDER BY sort_order ASC LIMIT 10');
        $fs->execute(['id' => $id]);
        $features = array_column($fs->fetchAll(), 'feature_text');
    } elseif ($type === 'product') {
        $fs = get_db()->prepare('SELECT feature_text FROM product_features WHERE product_id = :id ORDER BY sort_order ASC LIMIT 10');
        $fs->execute(['id' => $id]);
        $features = array_column($fs->fetchAll(), 'feature_text');
    }
    $featureList = '';
...
function normalize_bd_whatsapp(string $raw): string
{
    ...
    // ৮৮০ প্রিফিক্স থাকলে সরিয়ে দিই — নিচে একই যাচাইয়ে পাঠানোর জন্য
    if (substr($d, 0, 3) === '880') {
        $d = substr($d, 3);
    } elseif ($d[0] === '0') {
        $d = substr($d, 1);     // 01... → 1...
    }
    // ⚠️ বাংলাদেশি মোবাইল সবসময় `1` দিয়ে শুরু, ঠিক ১০ ডিজিট (যেমন 1685793049)।
    // এর বাইরে হলে (টাইপো — বাড়তি/কম সংখ্যা) খালি ফেরে, যাতে ভুল নম্বরে বাটন না দেখায়।
    // (২০২৬-০৭-২১: ইউজার ভুলে ১২-ডিজিট নম্বর দিয়েছিলেন, আগে সেটা চুপচাপ পাস হয়ে "isn't on
    //  WhatsApp" এররে ঠেকত — এখন আগেই ধরা পড়ে।)
    if (!preg_match('/^1[3-9][0-9]{8}$/', $d)) {
        return '';
    }
    return '880' . $d;
}
...
function render_facebook_section(): string
{
    ...
    foreach ($posts as $p) {
        ...
        $out .= '<span class="fb-card-cta">'
             ...
             .  'ফেসবুকে দেখুন <span class="arr" aria-hidden="true">&rarr;</span></span>';
        $out .= '</div></a>';
    }
    $out .= '</div>';
